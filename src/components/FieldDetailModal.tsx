...
import { X, FileText, Calculator, TrendingUp, Info, AlertCircle } from 'lucide-react';
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                            <Badge variant="outline" className="flex items-center gap-1">
                                ...
                                {field.source === 'user_data' ? 'Persönliche Daten' :
                                        ...
                                        <TableBody>
                                            {transactions.map((transaction) => (
                                                <TableRow key={transaction.id}>
                                                    ...
                                                    <TableCell className="max-w-xs truncate" title={transaction.counterpartyField}>
                                                        {transaction.counterpartyField}
                                                    </TableCell>
                                                    <TableCell className={`font-medium ${transaction.BetragNumeric > 0 ? 'text-green-600' : 'text-red-600'
                                                        }`}>
                    ...
                    <Card className="mt-6">
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                <div>
                                    <h4 className="font-medium mb-2">Was bedeutet dieses Feld?</h4>
                                    <p className="text-sm text-muted-foreground">
                                        Feld {field.field} ({field.label}) ist ein {field.type}-Feld in Ihrer EÜR.
                                        {field.required ? ' Es handelt sich um ein Pflichtfeld, das für die Steuererklärung ausgefüllt werden muss.' : ' Dieses Feld ist optional.'}
                                    </p>
                                </div>

                                <div>
                                    <h4 className="font-medium mb-2">Wie wird der Wert bestimmt?</h4>
                                    <p className="text-sm text-muted-foreground">
                                ...
                                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                    ...
                                    <p className="text-sm text-blue-700">
                                        Überprüfen Sie alle automatisch ausgefüllten Felder auf Richtigkeit.
                                        Bei Abweichungen können Sie die zugrunde liegenden Transaktionen überarbeiten.
                                    </p>
                                </div>
                            </div>
                        </CardContent>
