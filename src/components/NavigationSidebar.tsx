                        ...
                        <div className="p-3 rounded-lg border border-border bg-background">
                            ...
                            <div className="space-y-1.5">
                                <div className="flex items-center justify-between text-xs">
                                    ...
                                    <span className="font-mono text-green-600">+{euerCalculation.totalIncome.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€</span>
                                </div>
                                <div className="flex items-center justify-between text-xs">
                                    <span className="truncate pr-2">Betriebsausgaben</span>
                                    <span className="font-mono text-red-600">-{Math.abs(euerCalculation.totalExpenses).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€</span>
                                </div>
                                <div className="flex items-center justify-between text-xs pt-1.5 border-t border-border/60">
                                    <span className="font-medium truncate pr-2">Gewinn / Verlust</span>
                                    <span className={`font-mono font-medium ${
                                        euerCalculation.profit >= 0 ? 'text-green-600' : 'text-red-600'
                                    }`}>
                                        {euerCalculation.profit >= 0 ? '+' : ''}{euerCalculation.profit.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€
                                    </span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="space-y-1.5">
                                <div className="flex items-center justify-between text-xs">
                                    ...
                                    <span className="font-mono text-green-600">
                                        {euerCalculation.privateDeposits === 0 ? '0,00€' : `+${euerCalculation.privateDeposits.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€`}
                                    </span>
                                </div>
                                <div className="flex items-center justify-between text-xs">
                                    <span className="truncate pr-2">Privatentnahmen</span>
                                    <span className="font-mono text-red-600">
                                        {euerCalculation.privateWithdrawals === 0 ? '0,00€' : `-${Math.abs(euerCalculation.privateWithdrawals).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€`}
                                    </span>
                                </div>
                                <div className="flex items-center justify-between text-xs pt-1.5 border-t border-border/60">
                                    <span className="font-medium truncate pr-2">Summe</span>
                                    <span className={`font-mono font-medium ${
                                        (euerCalculation.privateDeposits - Math.abs(euerCalculation.privateWithdrawals)) >= 0 ? 'text-green-600' : 'text-red-600'
                                    }`}>
                                        {((euerCalculation.privateDeposits - Math.abs(euerCalculation.privateWithdrawals)) >= 0 ? '+' : '')}{(euerCalculation.privateDeposits - Math.abs(euerCalculation.privateWithdrawals)).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}€
                                    </span>
                                </div>
                            </div>
                        </div>
