...
const TransactionRow = React.memo<TransactionRowProps>(
	({
		...
		return (
			<tr
				className={`border-b border-border ${isPrivate ? "bg-private/5" : ""}`}
			>
				<td className="p-3 text-foreground text-sm whitespace-nowrap text-left font-mono text-center">
					{formattedDate}
				</td>
				<td
					className="p-3 text-foreground truncate max-w-0 text-left"
				...
				<td
					className={`p-3 whitespace-nowrap text-right text-sm font-mono ${amountColorClass}`}
				>
					{formattedAmount} €
				</td>
				<td
					className="p-3 text-muted-foreground truncate max-w-0 text-left"
