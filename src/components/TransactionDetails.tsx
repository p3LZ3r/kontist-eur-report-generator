...
const TransactionDetails: React.FC<TransactionDetailsProps> = ({
  ...
  rows.push(
    <tr className="border-gray-200 border-b bg-gray-50" key="summary">
      <td className="w-20 px-2 py-2 text-left">
        {" "}
        {/* Feste Breite wie bei Transaktionen */}
              ...
              {Object.keys(vatRates).length > 1 && (
                <div className="mt-0.5 flex flex-wrap justify-end gap-x-2 gap-y-0.5">
                  {Object.entries(vatRates)
                    ...
                    .map(([rate, amount]) => (
                      <span className="font-mono text-[10px]" key={rate}>
  ...
  transactions.forEach((transaction, index) => {
    rows.push(
      <tr
        className="border-gray-100 border-b transition-colors hover:bg-muted/10"
