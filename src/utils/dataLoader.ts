...
export function createCategoryMapping(accounts: Account[]): KontenrahmenData {
  ...
  leafAccounts.forEach(account => {
    ...
    if (categoryKey) {
      mapping[categoryKey] = {
        name: account.name,
        type: account.type === 'Ertrag' ? 'income' :
              account.type === 'Aufwand' ? 'expense' :
              account.type === 'Eigenkapital' ? 'private' : 'expense',
...
function mapAccountToCategory(accountName: string, accountType: string): string | null {
  ...
  if (accountType === 'Aufwand') {
    ...
    if (name.includes('soziale aufwendungen') || name.includes('arbeitgeberanteile')) return 'expense_social_employer';
    ...
    if (name.includes('strom') || name.includes('gas') || name.includes('wasser')) return 'expense_utilities';
    if (name.includes('telefon') || name.includes('telekom')) return 'expense_phone';
    if (name.includes('büromaterial') || name.includes('bürobedarf')) return 'expense_office_supplies';
    if (name.includes('werbekosten') || name.includes('werbung')) return 'expense_advertising_print';
