...
function inferTypeFromGroup(
  group: string,
  nr: string
): NormalizedElsterField["type"] {
  ...
  if (group === "Rücklagen, stille Reserven und Ausgleichsposten")
    return "expense";
...
export function loadNormalizedElsterFields(): NormalizedElsterField[] {
  ...
  Object.keys(
    eurFields as Record<string, Array<{ nr: string; name: string }>>
  ).forEach((groupName) => {
    const entries =
      (eurFields as Record<string, Array<{ nr: string; name: string }>>)[
        groupName
    ...
    for (const entry of entries) {
      ...
      const autoCalculated =
        AUTO_CALC_FIELDS.has(nr) || type === "vat" || type === "total";
