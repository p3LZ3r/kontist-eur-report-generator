...
import eurFields from "../../eur-fields-2024.json" with { type: "json" };
...
const EXCLUDED_GROUPS = new Set<string>(["Allgemeine Angaben", "Mitwirkung"]);
...
export function loadNormalizedElsterFields(): NormalizedElsterField[] {
  ...
  Object.keys(eurFields as Record<string, Array<{ nr: string; name: string }>>).forEach(
    (groupName) => {
      const entries =
        (eurFields as Record<string, Array<{ nr: string; name: string }>>)[groupName] || [];
  ...
  deduped.sort((a, b) => Number.parseInt(a.field, 10) - Number.parseInt(b.field, 10));
