...
export const getDemoInfo = () => {
  ...
  const totalIncome = transactions.filter(t => t.BetragNumeric > 0).reduce((sum, t) => sum + t.BetragNumeric, 0);
  const totalExpenses = Math.abs(transactions.filter(t => t.BetragNumeric < 0).reduce((sum, t) => sum + t.BetragNumeric, 0));
