...
```bash
npm run dev          # Start Vite Plus development server at localhost:5173
...
This is a React/TypeScript application for generating German EUR (Einnahmen-Uberschuss-Rechnung) reports from Kontist and Holvi bank transaction data. The application processes CSV exports, categorizes transactions according to SKR accounting standards, and generates ELSTER-compliant tax reports.

### Core Domain Logic

**EUR Calculations** (`src/utils/euerCalculations.ts`)
...
**ELSTER-SKR Mapping** (`src/utils/elsterSkrMapping.ts`)

- Maps SKR account codes to ELSTER form fields
- Comprehensive mapping based on DATEV documentation
...
3. **EUR Calculation**: Transactions + categories -> EUR calculation with VAT separation
4. **ELSTER Export**: EUR data -> Multiple export formats (CSV, JSON, PDF reports)

### Key Data Types (`src/types/index.ts`)

- `Transaction`: Bank transaction with categorization
- `EuerCalculation`: Complete EUR results including VAT breakdown
...
- Transaction categorization interface with TanStack Table
...
- **Legacy Text Export**: Traditional EUR summary format
...
- Automatic ELSTER VAT field population
...
- **6560** = "Leasingfahrzeugkosten"
- **6640** = "Bewirtungskosten" (not 6630)
...
- **Vite Plus** - Build tooling (vp commands)
...
│   ├── euer-generator/  # Main app components
...
- **State**: React useState/useMemo for local state
- **Processing**: Pure functions, immutable data patterns
- **Errors**: Try-catch with German user messages
- **Types**: Strict TypeScript with comprehensive interfaces