...
import { parseKontistCSV, categorizeTransaction } from './src/utils/transactionUtils.js';
import { calculateEuer } from './src/utils/euerCalculations.js';
import { populateAllElsterFields } from './src/utils/euerCalculations.js';
...
try {
    ...
    console.log('✅ CSV parsing successful:', transactions.length, 'transactions');
    ...
    console.log('   Income total:', Object.values(euerResult.income).reduce((a, b) => a + b, 0));
    console.log('   Expense total:', Object.values(euerResult.expenses).reduce((a, b) => a + b, 0));
    
    // 5. Populate ELSTER fields
    const { fieldValues } = populateAllElsterFields(transactions, categories, false);
    console.log('✅ ELSTER field population successful:', fieldValues.length, 'fields');
